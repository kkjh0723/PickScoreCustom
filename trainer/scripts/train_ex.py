...
from trainer.scripts.train import load_optimizer, load_scheduler, load_task, verify_or_write_config
...
def load_dataloaders(cfg: DictConfig, cfg_c: DictConfig) -> Any:
    ...
    for split in [cfg.valid_split_name, cfg.test_split_name]:
    ...
    for split, key in [(cfg_c.valid_split_name, cfg_c.valid_split_key), (cfg_c.test_split_name, cfg_c.test_split_key)]:
        dataset = instantiate_with_cfg(cfg_c, split=split)
        should_shuffle = split == cfg_c.train_split_name
...
def main(cfg: TrainerConfig) -> None:
    ...
    split2dataloader = load_dataloaders(cfg.dataset, cfg.dataset_custom)
    ...
    def evaluate():
        ...
        logger.info(f"*** Evaluating {cfg.dataset_custom.valid_split_name} ***")
        metrics = task.evaluate(model, criterion, split2dataloader[cfg.dataset_custom.valid_split_name])
        metrics = {f"{cfg.dataset_custom.valid_split_name}_{k}": v for k, v in metrics.items()}
        accelerator.update_metrics(metrics)
    ...
    logger.info(f"num. valid custom examples: {len(split2dataloader[cfg.dataset_custom.valid_split_key].dataset)}")
    logger.info(f"num. test custom examples: {len(split2dataloader[cfg.dataset_custom.test_split_key].dataset)}")
